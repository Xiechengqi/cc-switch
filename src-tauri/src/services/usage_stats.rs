...
/// 把 33 列的查询结果映射为 `RequestLogDetail`。
///
/// 调用方的 SELECT **必须**按以下顺序返回 33 列：
...
///  share_id, share_name, user_email, data_source, pricing_model,
...
fn row_to_request_log_detail(row: &rusqlite::Row<'_>) -> rusqlite::Result<RequestLogDetail> {
    Ok(RequestLogDetail {
        ...
        pricing_model: row.get(31)?,
        input_token_semantics: row.get::<_, i64>(32)?,
    })
}
...
impl Database {
    ...
    pub fn get_request_logs(
        ...
        if let Some(ref share_id) = filters.share_id {
            conditions.push("l.share_id = ?".to_string());
            params.push(Box::new(share_id.clone()));
        }
        ...
        let sql = format!(
            "SELECT l.request_id, l.provider_id, {logs_pname} as provider_name, l.app_type, l.model,
                    ...
                    l.status_code, l.error_message, l.created_at, l.share_id, l.share_name, l.user_email, l.data_source, l.pricing_model,
    ...
    pub fn get_request_detail(
        ...
        let detail_sql = format!(
            "SELECT l.request_id, l.provider_id, {detail_pname} as provider_name, l.app_type, l.model,
                    ...
                    status_code, error_message, created_at, l.share_id, l.share_name, l.user_email, l.data_source, l.pricing_model,
                    l.input_token_semantics
             FROM proxy_request_logs l
             LEFT JOIN providers p ON l.provider_id = p.id AND l.app_type = p.app_type
             WHERE l.request_id = ?"
