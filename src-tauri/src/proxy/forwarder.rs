...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        let codex_responses_base_is_full_endpoint =
        ...
        let response: ProxyResponse = loop {
            ...
            let custom_user_agent =
                if custom_user_agent.is_none() && codex_impersonate_claude_code {
            ...
            // impersonation is on; on the plain Codex→Anthropic path anthropic-version
            // is still required but no beta is sent.
            if (should_send_anthropic_headers || codex_responses_to_anthropic)
