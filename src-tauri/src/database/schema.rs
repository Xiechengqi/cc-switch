...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    30 => {
                        log::info!("迁移数据库从 v30 到 v31（添加项目 Profiles 表）");
                        Self::migrate_v30_to_v31(conn)?;
                        Self::set_user_version(conn, 31)?;
                    }
                    31 => {
                        log::info!("迁移数据库从 v31 到 v32（记录输入 token 缓存语义）");
    ...
    /// v26 -> v27：proxy_request_logs 增加 pricing_model 列；usage_daily_rollups
    /// 增加 pricing_model 维度（进入主键），便于审计路由接管下真实计价基准。
    ///
    /// 与上游 v10→v11 等价，但在本仓的迁移链中作为最末一步。SQLite 改主键
    /// 必须重建表；历史行的 pricing_model 不可知，填 ''。
    fn migrate_v26_to_v27(conn: &Connection) -> Result<(), AppError> {
        ...
        if !Self::table_exists(conn, "usage_daily_rollups")? {
            log::info!("v26 -> v27：usage_daily_rollups 不存在，跳过重建");
            return Ok(());
        }
        ...
        .map_err(|e| {
            AppError::Database(format!("v26 -> v27 重建 usage_daily_rollups 失败: {e}"))
        })?;
    ...
    fn migrate_v30_to_v31(conn: &Connection) -> Result<(), AppError> {
        ...
        .map_err(|e| AppError::Database(format!("v30 -> v31 创建 profiles 表失败: {e}")))?;
        Ok(())
    }

    /// v31 -> v32：记录 input_tokens 是否包含缓存写入。
    ///
...
mod tests {
    ...
    #[test]
    fn migrate_v31_to_v32_adds_input_token_semantics_columns() -> Result<(), AppError> {
        ...
        Database::set_user_version(&conn, 31)?;

        Database::apply_schema_migrations_on_conn(&conn)?;

        assert_eq!(Database::get_user_version(&conn)?, 32);
