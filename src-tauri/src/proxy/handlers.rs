...
struct ClaudeUsageLog {
    ...
    share_user_email: Option<String>,
    usage: TokenUsage,
    latency_ms: u64,
    status_code: u16,
    is_streaming: bool,
...
fn prepare_claude_usage_log(
    ...
    Some(ClaudeUsageLog {
        ...
        incoming_request_id: ctx.incoming_request_id.clone(),
        share_id: ctx.share_id.clone(),
        share_name: ctx.share_name.clone(),
        share_user_email: ctx.share_user_email.clone(),
        usage,
        ...
        let usage_collector = if usage_logging_enabled(state) {
            ...
            Some(SseUsageCollector::new(
                ...
                move |events, first_token_ms| {
                    ...
                    let outbound_model_for_log = fallback_model.clone();
...
pub async fn handle_images_generations(
    ...
    if !ctx.provider.is_codex_official_with_managed_auth() {
        return Ok(codex_image_error_response(
            ...
            "Image generation requires an OpenAI OAuth provider with a managed ChatGPT account.",
        ));
    }
