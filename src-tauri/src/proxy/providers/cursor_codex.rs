...
pub async fn forward_cursor_codex(
    ...
    match protocol {
        CursorProtocol::AgentService => {
            ...
            match response {
                ...
                Err(err) if should_fallback_to_chat_service(&mapped_body) => {
                    log::warn!("[CursorCodex] AgentService 失败，回退 ChatService: {err}");
                    let response = run_chat_service_codex(
                        &resolved_account,
                        &token,
                        headers,
                        &mapped_body,
        ...
        CursorProtocol::ChatService => run_chat_service_codex(
            &resolved_account,
            &token,
            headers,
            &mapped_body,
...
async fn run_chat_service_codex(
